...
export class PathMaterializer {
    static moveToGridMove(move: Move): GridMove {
        return {
    ...
    static calcCollisionType(gridMove: GridMove): CollisionType {
    ...
    static addToCollisions(collisions: string[], x: number, y: number) {
    ...
    static detectCollisions(
        ...
        const direction: [number, number] = this.directionToEdge(to);
        switch (collisionType) {
            // Horizontal
            case CollisionType.HORIZONTAL: {
                if (to.i < from.i) {
                    for (let i = from.i - 1; i > to.i; i--) {
                        this.addToCollisions(collisions, i, from.j);
                    }
            ...
            case CollisionType.VERTICAL: {
                if (to.j < from.j) {
                    for (let j = from.j - 1; j > to.j; j--) {
                        this.addToCollisions(collisions, from.i, j);
                    }
                    if (collisions.length > 0) {
                        this.addToCollisions(
                            collisions,
    ...
    static findShimmyLocation(
        ...
        switch (collisionType) {
            // Horizontal
            case CollisionType.HORIZONTAL: {
                const direction: [number, number] = this.directionToEdge(
                    move.to,
                );
    ...
    static constructFinalCommand(
        ...
        const dirToEdge = this.directionToEdge(from);
        ...
        if (mainPiece !== undefined) {
            ...
            if (
                ...
                const mainDrive1 = this.constructDriveCommand(
                    mainPiece,
                    pos1,
                    null,
                );
                const mainDrive2 = this.constructDriveCommand(
                    mainPiece,
                    pos2,
                    pos1,
                );
                const mainDrive3 = this.constructDriveCommand(
                    mainPiece,
                    pos3,
                    pos2,
                );
                const mainTurn1 = this.constructRotateCommand(
                    mainPiece,
                    pos1,
                    null,
                );
                const mainTurn2 = this.constructRotateCommand(
                    mainPiece,
                    pos2,
                    pos1,
                );
                const mainTurn3 = this.constructRotateCommand(
                    mainPiece,
                    pos3,
                    pos2,
                );
                ...
                const mainDrive = this.constructDriveCommand(
                    mainPiece,
                    pos,
                    null,
                );
                const mainTurn = this.constructRotateCommand(
                    mainPiece,
                    pos,
                    null,
                );
    ...
    static moveMainPiece(move: GridMove): MovePiece {
        const driveCommands: DriveCommand[] = [];
        const rotateCommands: ReversibleRobotCommand[] = [];
        const collisionType = this.calcCollisionType(move);
        const collisions: string[] = this.detectCollisions(move, collisionType);
        for (let i = 0; i < collisions.length; i++) {
            const pieceId = collisions[i];
            const location = this.findShimmyLocation(
                pieceId,
                move,
                collisionType,
            );
            driveCommands.push(
                this.constructDriveCommand(pieceId, location, null),
            );
            rotateCommands.push(
                this.constructRotateCommand(pieceId, location, null),
            );
        }
        return this.constructFinalCommand(
            move,
            driveCommands,
            rotateCommands,
            collisionType,
     ...
     */
    static moveToDeadZone(origin: GridIndices): GridMove {
        const aboveMove = {
            from: origin,
        ...
        const aboveCollision = this.detectCollisions(
            aboveMove,
            this.calcCollisionType(aboveMove),
        );
        const belowCollision = this.detectCollisions(
            belowMove,
            this.calcCollisionType(belowMove),
        );
        const leftCollision = this.detectCollisions(
            leftMove,
            this.calcCollisionType(leftMove),
        );
        const rightCollision = this.detectCollisions(
            rightMove,
            this.calcCollisionType(rightMove),
        );
    ...
    static directionToEdge(position: GridIndices) {
        let x = 0;
        let y = 0;
    ...
    static returnToHome(from: GridIndices, id: string): SequentialCommandGroup {
        //const capturedPiece: GridIndices = GridIndices.squareToGrid(from);
        const home: GridIndices = robotManager.getRobot(id).homeIndices;
        const fastestMoveToDeadzone = this.moveToDeadZone(from);
        const toDeadzone = this.moveMainPiece(fastestMoveToDeadzone);
        ...
        const startInArray = this.findGridIndicesInArray(
            arrayOfDeadzone,
            startInDeadzone,
        );
        const endInArray = this.findGridIndicesInArray(
            arrayOfDeadzone,
            finalDestination,
        );
        ...
        moveCommands.push(
            new AbsoluteMoveCommand(
                id,
                new Position(home.i + 0.5, home.j + 0.5),
            ),
        );
     ...
     */
    static moveAllRobotsToDefaultPositions(
        ...
        for (const robot of sortedRobots) {
            const robotCommands = this.generateRobotPathToDefault(
                robot,
                defaultPositions,
            );
     ...
     */
    static moveAllRobotsToConfigDefaultPositions(): SequentialCommandGroup {
        ...
        return this.moveAllRobotsToDefaultPositions(defaultPositions);
    }
     ...
     */
    static moveAllRobotsHomeToDefaultOptimized(): SequentialCommandGroup {
        ...
        while (
            ...
            const pick = (arr: PawnInfo[] | undefined) => {
                ...
                const dzStart = this.moveToDeadzoneFromHome(pawn.start);
     ...
     */
    static generateRobotPathToDefault(
        robot: Robot,
        defaultPositions: Map<string, GridIndices>,
    ): Command[] {
        ...
        if (!defaultPos) {
            throw new Error(
                `No default position specified for robot ${robot.id}`,
            );
        }

        // Move from home to deadzone
        const deadzonePos = this.moveToDeadzoneFromHome(currentPos);
        commands.push(
            new AbsoluteMoveCommand(
                robot.id,
        ...
        if (!deadzonePos.equals(topOfColumnPos)) {
            const deadzoneCommands = this.generateDeadzonePath(
                robot.id,
                deadzonePos,
                topOfColumnPos,
            );
        ...
        commands.push(
            new AbsoluteMoveCommand(
                robot.id,
                Position.fromGridIndices(defaultPos),
            ),
        );
    ...
    static generateDeadzonePath(
        ...
        const endInArray = this.findGridIndicesInArray(arrayOfDeadzone, endPos);
        ...
        while (i !== endInArray) {
            ...
            const nextCorner = this.findNextCornerOrEnd(
                i,
                endInArray,
                direction,
            );
     ...
     */
    static moveToDeadzoneFromHome(homePos: GridIndices): GridIndices {
    ...
    static materializePath(move: Move): Command {
        if (
            ...
            if (capturePiece !== undefined) {
                ...
                const captureCommand = this.returnToHome(
                    captureSquare,
                    capturePiece,
                );
                const mainCommand = this.moveMainPiece(
                    this.moveToGridMove(move),
                );
            ...
            let rookMove3;
            let rookPiece;
            if (this.moveToGridMove(move).from.j === 2) {
                rookPiece = robotManager.getRobotAtIndices(
                    new GridIndices(2, 2),
                );
                kingMove = new AbsoluteMoveCommand(
                    robotManager.getRobotAtIndices(
                        this.moveToGridMove(move).from,
                    ).id,
            ...
            return this.moveMainPiece(this.moveToGridMove(move));
        }
    }
}
