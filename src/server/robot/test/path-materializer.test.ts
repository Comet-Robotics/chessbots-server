...
test.each([
    ...
    if (move.from.j === move.to.j) {
        ...
        Math.abs(move.from.i - move.to.i) === Math.abs(move.from.j - move.to.j)
    ) {
...
const nullRobot = new Robot(
    "null",
    { i: 0, j: 0 } as GridIndices,
    { i: 0, j: 0 } as GridIndices,
);
...
it("Test collision adding", () => {
    //true case mock
    const isSpy = vi
        .spyOn(robotManager, "isRobotAtIndices")
        .mockReturnValue(true);
    const getSpy = vi
        .spyOn(robotManager, "getRobotAtIndices")
        .mockImplementation((grid: GridIndices) => {
            if (grid.i === 1 && grid.j === 1)
                return new Robot(
                    "asdf",
                    { i: 2, j: 2 } as GridIndices,
                    { i: 0, j: 0 } as GridIndices,
                );
            return nullRobot;
        });
...
describe("Test collision detection", () => {
    const getMock = (grid: GridIndices) => {
        if (grid.i === 6 && grid.j === 5) {
            return new Robot(
                "right",
                { i: 6, j: 5 } as GridIndices,
                { i: 0, j: 0 } as GridIndices,
            );
        } else if (grid.i === 4 && grid.j === 5) {
            return new Robot(
                "left",
                { i: 4, j: 5 } as GridIndices,
                { i: 0, j: 0 } as GridIndices,
            );
        } else if (grid.i === 5 && grid.j === 6) {
            return new Robot(
                "up",
                { i: 4, j: 6 } as GridIndices,
                { i: 0, j: 0 } as GridIndices,
            );
        } else if (grid.i === 5 && grid.j === 4) {
            return new Robot(
                "down",
                { i: 5, j: 4 } as GridIndices,
                { i: 0, j: 0 } as GridIndices,
            );
    ...
    it("horizontal test", () => {
        const addSpy = vi
            .spyOn(PathMaterializer, "addToCollisions")
            .mockImplementation(addMock);

        let starting = {
            from: { i: 5, j: 5 } as GridIndices,
            to: { i: 7, j: 5 } as GridIndices,
        } as GridMove;
        expect(
            PathMaterializer.detectCollisions(
                starting,
    ...
    it("vertical test", () => {
        ...
        let starting = {
            from: { i: 5, j: 5 } as GridIndices,
            to: { i: 5, j: 7 } as GridIndices,
        } as GridMove;
        expect(
            PathMaterializer.detectCollisions(starting, CollisionType.VERTICAL),
        ).toContain("up");

        starting = {
            from: { i: 5, j: 5 } as GridIndices,
            to: { i: 5, j: 3 } as GridIndices,
        } as GridMove;
        expect(
            PathMaterializer.detectCollisions(starting, CollisionType.VERTICAL),
        ).toContain("down");
    ...
    it("diagonal tests", () => {
        ...
        let starting = {
            from: { i: 5, j: 5 } as GridIndices,
            to: { i: 3, j: 7 } as GridIndices,
        } as GridMove;
        expect(
            PathMaterializer.detectCollisions(starting, CollisionType.DIAGONAL),
        ).containSubset(["up", "left"]);
        //top right
        starting = {
            from: { i: 5, j: 5 } as GridIndices,
            to: { i: 7, j: 7 } as GridIndices,
        } as GridMove;
        expect(
            PathMaterializer.detectCollisions(starting, CollisionType.DIAGONAL),
        ).containSubset(["up", "right"]);
        //bottom left
        starting = {
            from: { i: 5, j: 5 } as GridIndices,
            to: { i: 3, j: 3 } as GridIndices,
        } as GridMove;
        expect(
            PathMaterializer.detectCollisions(starting, CollisionType.DIAGONAL),
        ).containSubset(["down", "left"]);
        //bottom right
        starting = {
            from: { i: 5, j: 5 } as GridIndices,
            to: { i: 7, j: 3 } as GridIndices,
        } as GridMove;
        expect(
            PathMaterializer.detectCollisions(starting, CollisionType.DIAGONAL),
        ).containSubset(["down", "right"]);
    ...
    it("horse tests", () => {
        ...
        let starting = {
            from: { i: 5, j: 5 } as GridIndices,
            to: { i: 3, j: 6 } as GridIndices,
        } as GridMove;
        expect(
            PathMaterializer.detectCollisions(starting, CollisionType.HORSE),
        ).containSubset(["left", "topleft"]);
        //left down
        starting = {
            from: { i: 5, j: 5 } as GridIndices,
            to: { i: 3, j: 4 } as GridIndices,
        } as GridMove;
        expect(
            PathMaterializer.detectCollisions(starting, CollisionType.HORSE),
        ).containSubset(["left", "bottomleft"]);
        //right up
        starting = {
            from: { i: 5, j: 5 } as GridIndices,
            to: { i: 7, j: 6 } as GridIndices,
        } as GridMove;
        expect(
            PathMaterializer.detectCollisions(starting, CollisionType.HORSE),
        ).containSubset(["right", "topright"]);
        //right down
        starting = {
            from: { i: 5, j: 5 } as GridIndices,
            to: { i: 7, j: 4 } as GridIndices,
        } as GridMove;
        expect(
            PathMaterializer.detectCollisions(starting, CollisionType.HORSE),
        ).containSubset(["right", "bottomright"]);

        //up left
        starting = {
            from: { i: 5, j: 5 } as GridIndices,
            to: { i: 4, j: 7 } as GridIndices,
        } as GridMove;
        expect(
            PathMaterializer.detectCollisions(starting, CollisionType.HORSE),
        ).containSubset(["up", "topleft"]);
        //up right
        starting = {
            from: { i: 5, j: 5 } as GridIndices,
            to: { i: 6, j: 7 } as GridIndices,
        } as GridMove;
        expect(
            PathMaterializer.detectCollisions(starting, CollisionType.HORSE),
        ).containSubset(["up", "topright"]);
        //down left
        starting = {
            from: { i: 5, j: 5 } as GridIndices,
            to: { i: 4, j: 3 } as GridIndices,
        } as GridMove;
        expect(
            PathMaterializer.detectCollisions(starting, CollisionType.HORSE),
        ).containSubset(["down", "bottomleft"]);
        //down right
        starting = {
            from: { i: 5, j: 5 } as GridIndices,
            to: { i: 6, j: 3 } as GridIndices,
        } as GridMove;
        expect(
            PathMaterializer.detectCollisions(starting, CollisionType.HORSE),
        ).containSubset(["down", "bottomright"]);
...
test.each([
    ...
    expect(PathMaterializer.directionToEdge(pos)).toEqual(
...
describe("Test shimmy location", () => {
    ...
    it("horizontal tests", () => {
        ...
        //right
        let starting = {
            from: { i: 4, j: 5 } as GridIndices,
            to: { i: 6, j: 5 } as GridIndices,
        } as GridMove;
        let output = PathMaterializer.findShimmyLocation(
            "asdf",
            starting,
            CollisionType.HORIZONTAL,
        );
        ...
        //left
        starting = {
            from: { i: 6, j: 5 } as GridIndices,
            to: { i: 4, j: 5 } as GridIndices,
        } as GridMove;
        output = PathMaterializer.findShimmyLocation(
            "asdf",
            starting,
            CollisionType.HORIZONTAL,
        );
        ...
        //right
        starting = {
            from: { i: 4, j: 4 } as GridIndices,
            to: { i: 6, j: 4 } as GridIndices,
        } as GridMove;
        output = PathMaterializer.findShimmyLocation(
            "asdf",
            starting,
        ...
        //left
        starting = {
            from: { i: 6, j: 4 } as GridIndices,
            to: { i: 4, j: 4 } as GridIndices,
        } as GridMove;
        output = PathMaterializer.findShimmyLocation(
            "asdf",
            starting,
    ...
    it("vertical tests", () => {
        ...
        //up
        let starting = {
            from: { i: 5, j: 4 } as GridIndices,
            to: { i: 5, j: 6 } as GridIndices,
        } as GridMove;
        let output = PathMaterializer.findShimmyLocation(
            "asdf",
            starting,
        ...
        //down
        starting = {
            from: { i: 5, j: 6 } as GridIndices,
            to: { i: 5, j: 4 } as GridIndices,
        } as GridMove;
        output = PathMaterializer.findShimmyLocation(
            "asdf",
            starting,
